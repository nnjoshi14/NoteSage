...
class OfflineCache {
    ...
    `);
        stmt.run(noteData.id, noteData.title, noteData.content, noteData.category, JSON.stringify(noteData.tags), noteData.folder_path, noteData.scheduled_date, noteData.is_archived ? 1 : 0, noteData.is_pinned ? 1 : 0, noteData.is_favorite ? 1 : 0, noteData.created_at, noteData.updated_at, noteData.version, noteData.server_id, noteData.sync_status, noteData.last_modified_locally);
    ...
    `);
        stmt.run(personData.id, personData.name, personData.email, personData.phone, personData.company, personData.title, personData.linkedin_url, personData.avatar_url, personData.notes, personData.created_at, personData.updated_at, personData.server_id, personData.sync_status, personData.last_modified_locally);
        // Add to offline queue if not synced
    ...
    `);
        stmt.run(todoData.id, todoData.note_id, todoData.todo_id, todoData.text, todoData.is_completed ? 1 : 0, todoData.assigned_person_id, todoData.due_date, todoData.created_at, todoData.updated_at, todoData.server_id, todoData.sync_status, todoData.last_modified_locally);
        // Add to offline queue if not synced
    ...
    async addToOfflineQueue(operation, tableName, recordId, data) {
        if (!this.db)
            throw new Error('Database not initialized');
        const now = new Date().toISOString();
        const stmt = this.db.prepare(`
    ...
    async updateOfflineQueueError(id, error) {
        if (!this.db)
            throw new Error('Database not initialized');
        const stmt = this.db.prepare(`
    ...
    async updateSyncMetadata(tableName, lastSync, syncToken) {
        if (!this.db)
            throw new Error('Database not initialized');
    ...
    async getCacheStats() {
        ...
        const lastCleanupRow = lastCleanupStmt.get('last_cleanup');
    ...
    getPendingChangesCount(tableName) {
        if (!this.db)
            return 0;
