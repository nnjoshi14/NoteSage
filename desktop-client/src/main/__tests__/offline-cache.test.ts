...
describe('OfflineCache', () => {
  ...
  describe('Note operations', () => {
    it('should save a note to cache', async () => {
      const noteData: Partial<CachedNote> = {
        title: 'Test Note',
      ...
      (cache.saveNote as jest.Mock).mockResolvedValue(expectedNote);
  ...
  describe('Person operations', () => {
    it('should save a person to cache', async () => {
      const personData: Partial<CachedPerson> = {
        name: 'John Doe',
    ...
    it('should get all people from cache', async () => {
      ...
      expect(people.map((p: any) => p.name).sort()).toEqual(['Person 1', 'Person 2']);
    });
  });

  describe('Todo operations', () => {
    it('should save a todo to cache', async () => {
      const todoData: Partial<CachedTodo> = {
        note_id: 'note-123',
        todo_id: 't1',
        text: 'Complete task',
        is_completed: false,
